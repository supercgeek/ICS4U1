...
if ($_GET['edit'] == "yes") {
	...
	while ($myrow = mysql_fetch_array($result))  { 
		...
		<form action='updating-records-fastest-cars.php' method='GET'>
			<input class = 'special' type='hidden' name='selected_id' value='" . $myrow['id'] ."'></input><BR>
			<input class = 'special' type='text' name='car' value='" . $myrow['car'] ."'></input><BR>
			<input class = 'special' type='text' name='top_speed' value='" . $myrow['top_speed'] ."'></input><BR>
			<input class = 'special'  type='text' name='production_year' value='" . $myrow['production_year'] ."'></input><BR>
			<input class = 'special'  type='submit' name='submitPressedForSure' value='Update Now!'></input>
		</form>
		<form class = 'special'  action='updating-records-fastest-cars.php'><input type='submit' name='cancel' value='Cancel'></input></form>
		";
	}
}
...
//	echo "ID CHECK: " .  $selected_id;
	if ($_GET['confirm'] == "yes") {
		...
		$queryDelete = "DELETE FROM fastest_cars WHERE id='" . $selected_id . "'";
...
<form action='' method='POST'><tr>
	<td>[N/A]</td><td><input type='text' placeholder='Enter Model Value' required name='modelVal' value=''></td><td><input type='number' min = '0' placeholder='Enter Top-Speed Value' required name='topSpeedVal' value=''></td><td><input type='number' min = '0' placeholder='Enter Year Value' required name='yearVal' value=''></td><td>[N/A]</td><input class='special' type='submit' name='submitBtn' value='Click to Insert New Record'><BR>
</tr>";
echo "<BR>";
while ($myrow = mysql_fetch_array($result))  { 
